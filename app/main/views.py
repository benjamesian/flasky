...
def index():
    ...
    comments = map(lambda x: x.comments, posts)
...
def edit_profile_admin(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('You have updated a profile')
...
def follow(username):
    ...
    db.session.commit()
    flash('You are now following {:s}.'.format(username))
    return redirect(url_for('.user', username=username))
...
def unfollow(username):
    ...
    db.session.commit()
    flash('You are no longer following {:s}'.format(username))
    return redirect(url_for('.user', username=username))
